...
const ExpensesCustom = () => {
    ...
    const fetchExpenses = async () => {
        setLoading(true);
        try {
            const response = await api.get("/expenses/custom"); // ✅ Use central API
            ...
            setExpenses([]); // ✅ Prevent UI break if API fails
        }
        setLoading(false);
    };
    ...
    const handleSaveExpense = async () => {
        try {
            if (isEditMode) {
                await api.put(`/expenses/${selectedExpense.id}`, selectedExpense); // ✅ Use central API
        ...
        <div className="border-t bg-white border-gray-200 p-4 rounded-t-md dark:bg-gray-800 dark:text-white">
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-2">
                    <ExportSearchControls searchQuery={searchQuery} setSearchQuery={setSearchQuery} fileName="Custom_Expenses" />
                </div>
            </div>
                        ...
                        <tbody>
                            {paginatedExpenses.length > 0 ? (
                                paginatedExpenses.map((expense) => (
                                    <tr key={expense._id || expense.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        {columns.map((col) =>
                                                ...
                                                <td key={col.key} className="px-6 py-4 text-sm">
                                                    {col.key === "action" ? (
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            {/* Delete Button */}
                                                            <button onClick={() => handleDeleteExpense(expense._id || expense.id)} className="p-1 rounded hover:bg-red-500 hover:text-white">
                                                                <SlClose size={20} />
                                                            </button>
                                                        </div>
                                                    ) : (
